...
const WriteBlogModal = () => {
  ...
  const [isAddingCategory, setIsAddingCategory] = useState(false) // For toggling new category input
    ...
    <div className=' lg:min-w-[1024px] md:min-w-[624px] min-w-[364px] max-h-[100vh] top-5 lg:p-10 md:px-8 py-8 px-4 gap-4 flex flex-col '>
      ...
      <form onSubmit={addPostToFireStore}>
        ...
        <div className=' flex lg:flex-row  flex-col gap-3'>
          ...
          {!isAddingCategory ? (
            <select
              className="w-full bg-transparent font-normal text-neutral-950 outline-none border-b-2 border-slate-200 lg:text-xl text-lg font-newsletter placeholder:font-newsletter focus:border-indigo-400 duration-300"
            ...
            >
              ...
              {availableCategories.map(cat => (
                <option key={cat.id} value={cat.data.topicName}>{cat.data.topicName}</option>
              ))}
        ...
        <div className='w-full max-w-full py-4'>
          <EditorContent editor={editor} className=' w-full border p-6 focus-visible:border-gray-500 font-newsletter border-gray-300 rounded -md outline-none' />
