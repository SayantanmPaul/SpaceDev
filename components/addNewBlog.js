...
const AddNewBlogModel = ({ closemodel }) => {
    ...
    return (
        <div className=' fixed top-0 left-0 right-0 bottom-0 backdrop-brightness-50 backdrop-blur flex items-center justify-center shadow-2xl'>
            <div className={`modal-container lg:rounded-3xl overflow-hidden flex flex-col lg:h-auto h-full max-w-4xl w-full bg-white p-8 gap-6 overflow-y-scroll no-scrollbar ${isClosing ? 'animate-fade-slideout' : 'animate-fade-slideup'} `}>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-8 w-full'>
                    <div className={`step-container ${direction}`}>
                        {step === 1 && (
                            <div className='step-content flex flex-col gap-8'>
                                <input
                                    type="text"
                                    placeholder='Title*'
                                    {...register('title', {
                                ...
                                />
                                {errors.title && (
                                    <p className='text-rose-500 text-md'>{errors.title.message}</p>
                                )}
                                <input
                                    type="url"
                                ...
                                />
                                {errors.bannerImage && (
                                    <p className='text-rose-500 text-md'>{errors.bannerImage.message}</p>
                                )}
                                <div className='flex flex-col lg:flex-row gap-4 w-full items-start justify-between'>
                                    ...
                                    {errors.selectedCategory && (
                                        <p className='text-rose-500 text-md'>{errors.selectedCategory.message}</p>
                                    )}
                                ...
                                <textarea
                                    ...
                                    {...register('brief', {
                                        required: 'brieft about a blog is required'
                                    })}
                                ...
                                />
                                {errors.brief && (
                                    <p className='text-rose-500 text-md'>{errors.brief.message}</p>
                                )}
                        ...
                        {step === 2 && (
                            <div className='step-content w-full flex flex-col gap-6'>
                                <div className='h-96 w-full container border rounded-lg p-5 overflow-y-scroll' id='editorjs'>
                                    <Editor
                                        data={content}
